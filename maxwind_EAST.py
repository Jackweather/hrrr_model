...
BASE_DIR = "/var/data"
RUN_ROOT_DIR = os.path.join(BASE_DIR, "maxwind_EAST")
...
wind_levels = list(range(0, 41, 2))
wind_tick_levels = wind_levels

# Dramatic colormap for max wind speed
wind_colors = [
    "#ffffff",  # 0 - calm
    "#c0f0ff",  # 2
    "#80e0ff",  # 4
...
wind_norm = BoundaryNorm(wind_levels, wind_cmap.N)
...
def get_wind_variable(dataset, var_names):
...
def plot_max_wind(uw_path, vw_path, step, run_time, region_name):
    ds_uw = ds_vw = None
    figure = None
    try:
        ...
        ds_uw = xr.open_dataset(uw_path, engine="cfgrib", chunks={})
        ds_vw = xr.open_dataset(vw_path, engine="cfgrib", chunks={})
        uw_field = get_wind_variable(ds_uw, ["maxuw", "uwind", "u", "uw"])  # Try common names
        vw_field = get_wind_variable(ds_vw, ["maxvw", "vwind", "v", "vw"])
        latitude_field, longitude_field = get_lat_lon_fields(ds_uw)
        if uw_field is None or vw_field is None or latitude_field is None or longitude_field is None:
            print("Required wind components or coordinates are not present in the dataset.")
            return None
        uw = uw_field.values.squeeze()
        vw = vw_field.values.squeeze()
        wind_speed = np.sqrt(uw ** 2 + vw ** 2)
        wind_speed = np.clip(wind_speed, wind_levels[0], wind_levels[-1])
        ...
        title = (
            f"HRRR 10 m Max Wind Speed - {region_config['title']}\n"
        ...
        colorbar = plt.colorbar(mesh, cax=colorbar_axis, orientation="horizontal", ticks=wind_tick_levels)
        colorbar.set_label("10 m Max Wind Speed (m/s)", fontsize=7, labelpad=2)
        ...
        png_path = os.path.join(png_dirs[region_name], f"hrrr_maxwind_{region_name}_{step:02d}.png")
        plt.savefig(png_path, bbox_inches="tight", dpi=300)
        print(f"Generated PNG: {png_path}")
        return png_path
    except Exception as exc:
        print(f"Error in plot_max_wind: {exc}")
        return None
    finally:
        if ds_uw is not None:
            ds_uw.close()
        if ds_vw is not None:
            ds_vw.close()
...
for step_group in forecast_steps:
    for step in step_group:
        uw_grib = get_hrrr_grib(most_recent_run_time, step, variable_uw)
        vw_grib = get_hrrr_grib(most_recent_run_time, step, variable_vw)
        if uw_grib and vw_grib:
            for region_name in ACTIVE_REGION_NAMES:
                plot_max_wind(uw_grib, vw_grib, step, most_recent_run_time, region_name)
        else:
            print(
                f"Skipping forecast hour {step:02d} for run {most_recent_run_time.strftime('%Y-%m-%d %HZ')} "
                "because the wind files are not available."
            )
        if uw_grib and os.path.exists(uw_grib):
            os.remove(uw_grib)
        if vw_grib and os.path.exists(vw_grib):
            os.remove(vw_grib)
        gc.collect()
    prune_old_runs(MAX_SAVED_RUNS, keep_run_id=current_run_id)

print("HRRR 10 m max wind processing complete.")
