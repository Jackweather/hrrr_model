...
BASE_DIR = "/var/data"
RUN_ROOT_DIR = os.path.join(BASE_DIR, "rh_EAST")
...
rh_levels = list(range(0, 101, 5))
rh_tick_levels = rh_levels
rh_colors = [
    "#ffffff", "#fefaf1", "#fcf2de", "#f8e9c6",
    "#f5deb3", "#ebebba", "#dcf2bc", "#caf8be",
    "#b9fbc0", "#9ff2a6", "#84e88b", "#68dc71",
    "#34c759", "#28b34c", "#1d9f40", "#118a33",
    "#006400", "#0a7a32", "#1e90ff", "#63b2ff",
]
...
def get_rh_variable(dataset):
    for variable_name in ("r2", "rh", "r"):
...
def plot_relative_humidity(rh_path, step, run_time, region_name):
    dataset = None
    figure = None
    try:
        ...
        dataset = xr.open_dataset(rh_path, engine="cfgrib", chunks={})
        rh_field = get_rh_variable(dataset)
        latitude_field, longitude_field = get_lat_lon_fields(dataset)
        if rh_field is None or latitude_field is None or longitude_field is None:
            print("Required relative humidity coordinates or variable are not present in the dataset.")
            return None

        rh_percent = np.clip(rh_field.values.squeeze(), rh_levels[0], rh_levels[-1])
        ...
        title = (
            f"HRRR 2 m Relative Humidity - {region_config['title']}\n"
        ...
        colorbar = plt.colorbar(mesh, cax=colorbar_axis, orientation="horizontal", ticks=rh_tick_levels)
        colorbar.set_label("2 m Relative Humidity (%)", fontsize=7, labelpad=2)
        ...
        png_path = os.path.join(png_dirs[region_name], f"hrrr_rh_{region_name}_{step:02d}.png")
        plt.savefig(png_path, bbox_inches="tight", dpi=300)
        print(f"Generated PNG: {png_path}")
        return png_path
    except Exception as exc:
        print(f"Error in plot_relative_humidity: {exc}")
        return None
    finally:
...
for step_group in forecast_steps:
    for step in step_group:
        rh_grib = get_hrrr_grib(most_recent_run_time, step, variable_rh)

        if rh_grib:
            for region_name in ACTIVE_REGION_NAMES:
                plot_relative_humidity(rh_grib, step, most_recent_run_time, region_name)
        else:
            print(
                f"Skipping forecast hour {step:02d} for run {most_recent_run_time.strftime('%Y-%m-%d %HZ')} "
                "because the relative humidity file is not available."
            )

        if rh_grib and os.path.exists(rh_grib):
            os.remove(rh_grib)
...
print("HRRR 2 m relative humidity processing complete.")
