...
BASE_DIR = "/var/data"
RUN_ROOT_DIR = os.path.join(BASE_DIR, "vis_EAST")
...
vis_tick_levels_miles = vis_levels_miles
vis_cmap = LinearSegmentedColormap.from_list(
    "vis_met_cmap",
    [
        "#3f007d",
        "#e31a1c",
...
vis_norm = BoundaryNorm(vis_levels_miles + [1000], vis_cmap.N)
...
def get_visibility_variable(dataset):
    for variable_name in ("vis", "visibility"):
        if variable_name in dataset:
            return dataset[variable_name]

    for data_var_name in dataset.data_vars:
        if "vis" in data_var_name.lower():
...
def plot_visibility(vis_path, step, run_time, region_name):
    dataset = None
    figure = None
    try:
        ...
        dataset = xr.open_dataset(vis_path, engine="cfgrib", chunks={}, backend_kwargs={"indexpath": ""})
        visibility_field = get_visibility_variable(dataset)
        latitude_field, longitude_field = get_lat_lon_fields(dataset)
        if visibility_field is None or latitude_field is None or longitude_field is None:
            print("Required visibility coordinates or variable are not present in the dataset.")
            return None

        visibility_meters = visibility_field.values.squeeze()
        ...
        title = (
            f"HRRR Surface Visibility - {region_config['title']}\n"
        ...
        colorbar = plt.colorbar(mesh, cax=colorbar_axis, orientation="horizontal", ticks=vis_tick_levels_miles)
        colorbar.set_label("Surface Visibility (miles)", fontsize=7, labelpad=2)
        ...
        png_path = os.path.join(png_dirs[region_name], f"hrrr_vis_{region_name}_{step:02d}.png")
        plt.savefig(png_path, bbox_inches="tight", dpi=300)
        print(f"Generated PNG: {png_path}")
        return png_path
    except Exception as exc:
        print(f"Error in plot_visibility: {exc}")
        return None
    finally:
...
for step_group in forecast_steps:
    for step in step_group:
        vis_grib = get_hrrr_grib(most_recent_run_time, step, variable_vis)

        if vis_grib:
            for region_name in ACTIVE_REGION_NAMES:
                plot_visibility(vis_grib, step, most_recent_run_time, region_name)
        else:
            print(
                f"Skipping forecast hour {step:02d} for run {most_recent_run_time.strftime('%Y-%m-%d %HZ')} "
                "because the visibility file is not available."
            )

        if vis_grib and os.path.exists(vis_grib):
            os.remove(vis_grib)
...
print("HRRR visibility processing complete.")
