...
BASE_DIR = "/var/data"
RUN_ROOT_DIR = os.path.join(BASE_DIR, "apcp_EAST")
...
precip_tick_levels = precip_levels
precip_colors = [
...
precip_cmap = LinearSegmentedColormap.from_list("precip_cmap", precip_colors, N=256)
precip_norm = BoundaryNorm(precip_levels, precip_cmap.N)


def format_precip_tick_label(value):
    if value >= 5 and float(value).is_integer():
...
def get_precip_variable(dataset):
    for variable_name in ("tp", "apcp", "unknown"):
...
def plot_precip(apcp_path, step, run_time, region_name):
    dataset = None
    figure = None
    try:
        ...
        precip_field = get_precip_variable(dataset)
        latitude_field, longitude_field = get_lat_lon_fields(dataset)
        if precip_field is None or latitude_field is None or longitude_field is None:
            print("Required precipitation coordinates or variable are not present in the dataset.")
            return None

        precip_mm = precip_field.values.squeeze()
        precip_inches = precip_mm * 0.0393700787
        ...
        title = (
            f"HRRR Total Precipitation - {region_config['title']}\n"
        ...
        colorbar = plt.colorbar(mesh, cax=colorbar_axis, orientation="horizontal", ticks=precip_tick_levels)
        colorbar.ax.set_xticklabels([format_precip_tick_label(value) for value in precip_tick_levels])
        colorbar.set_label("Accumulated Precipitation (inches)", fontsize=7, labelpad=2)
        ...
        png_path = os.path.join(png_dirs[region_name], f"hrrr_apcp_{region_name}_{step:02d}.png")
        plt.savefig(png_path, bbox_inches="tight", dpi=300)
        print(f"Generated PNG: {png_path}")
        return png_path
    except Exception as exc:
        print(f"Error in plot_precip: {exc}")
        return None
    finally:
...
for step_group in forecast_steps:
    for step in step_group:
        apcp_grib = get_hrrr_grib(most_recent_run_time, step, variable_apcp)

        if apcp_grib:
            for region_name in ACTIVE_REGION_NAMES:
                plot_precip(apcp_grib, step, most_recent_run_time, region_name)
        else:
            print(
                f"Skipping forecast hour {step:02d} for run {most_recent_run_time.strftime('%Y-%m-%d %HZ')} "
                "because the precipitation file is not available."
            )

        if apcp_grib and os.path.exists(apcp_grib):
            os.remove(apcp_grib)
...
print("HRRR accumulated precipitation processing complete.")
